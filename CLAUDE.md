...
- **doclang** uses `parser.New(logger).ParseDocument(content, path)`, the document-shaped mirror of `Parse`: it switches on the same frontmatter `mode:`, but between the two *document* dialects. `strict` gets `DocumentStrictParser` (declared `SECTION "Title"` blocks with `level:`/`id:`, never normalized); `flex`, `flex-full`, `auto` and a file with no frontmatter all get `DocumentFlexParser` — in documents those are synonyms, since there is one flex grammar and nothing to auto-detect. Flex treats `#`/`##`/`###` as a **section hierarchy** (subsections live inside their parent block, not as separate blocks), and the strict dialect deliberately produces the *same* AST shape — level-1 `SECTION` opens a `ContentBlock`, levels 2-6 become `<hN id>` heading elements inside it — so the renderer, the TOC and the xref/numbering transforms have no per-dialect branches. Each top-level `ContentBlock` is a **section**.
