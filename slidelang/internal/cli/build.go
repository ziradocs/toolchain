...
Examples:
  ...
  slidelang build slides.slidelang --enable-normalization --log-level warn`,
	...
	cmd.Flags().StringArrayVar(&opts.Filters, "filter", nil, "Path to an external filter binary that transforms the AST between parse and lint (repeatable; runs in the order given, each filter's output feeds the next). Communicates via JSON on stdin/stdout — see https://ziradocs.com/docs/architecture/json-ast-contract/")
