...
// writeFixture writes a minimal strict-mode .slidelang file at
// <dir>/fixture.slidelang, with or without the EXT001 waiver. mode: strict
// is used deliberately so `fmt` reformats rather than transpiles it,
// keeping the round-trip test focused on frontmatter preservation.
func writeFixture(t *testing.T, dir string, withWaiver bool) string {
	t.Helper()
	frontmatter := "mode: strict\ntitle: \"E2E Fixture\"\n"
	if withWaiver {
		frontmatter += waiverBlock(nil)
	}
	content := fmt.Sprintf("---\n%s---\n\nSLIDE content\n  title: \"Fixture\"\n  TEXT\n    Hello world.\n", frontmatter)
	path := filepath.Join(dir, "fixture.slidelang")
...
func writeFixtureScoped(t *testing.T, dir, name string, matching bool) string {
	...
	scope := path
	if !matching {
		scope = filepath.Join(dir, "does-not-exist.slidelang")
	}
	frontmatter := "mode: strict\ntitle: \"E2E Scope Fixture\"\n" + waiverBlock([]string{scope})
	content := fmt.Sprintf("---\n%s---\n\nSLIDE content\n  title: \"Fixture\"\n  TEXT\n    Hello world.\n", frontmatter)
	if err := os.WriteFile(path, []byte(content), 0644); err != nil {
...
// resolvePolicyFromFile parses path and resolves its embedded lint_policy,
// for the semantic (never byte-for-byte) before/after comparison the fmt
// round-trip test needs — fmt re-orders and re-quotes frontmatter keys, so
// comparing resolved structs is the only comparison that means what the
// test claims.
